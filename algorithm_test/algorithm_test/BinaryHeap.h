...
public:
	...
	void push(const pNode& node,int score[width][length])
	{
	...
	void pop(int score[width][length])
	{
		if(heap.size()-1 <= 0)
			return;
	...
	void update(const pNode& node,int score[width][length])
	{
